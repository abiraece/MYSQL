...
-- Solution:  
select BIDDER_ID,
sum(BID_STATUS = "won") as "Win number",
sum(BID_STATUS in ("Won","lost","bid","cancelled")) as "Total Bidding Count",
(sum(BID_STATUS = "won") / sum(BID_STATUS in ("Won","lost","bid","cancelled")))*100 as "Percentage_of_win"
...
a.STADIUM_NAME,
a.CITY,
count(b.MATCH_ID) as "Match conducted at each stadium"
from ipl_stadium a inner join ipl_match_schedule b
on a.stadium_id = b.stadium_id
group by b.stadium_id
order by a.STADIUM_ID;

-- 3.	Display total matches played, total matches won and total matches lost by team along with its team name.
-- Tables Used: 
select * from ipl_team_standings;
select * from ipl_team;
-- Solution: 
...
b.TEAM_NAME as "Team Name",
sum(a.MATCHES_PLAYED) as "Total Matches Played",
sum(a.MATCHES_WON) as "Total Matched Won",
sum(a.MATCHES_LOST) as "Total Matches Lost"
from ipl_team_standings a left join ipl_team b
on a.team_id = b.team_id
group by a.team_id;

-- 4.	Display the bowlers for Mumbai Indians team.
-- Tables used: 
select * from ipl_team_players;
select * from ipl_player;
-- Solution: 
select 
a.player_id as "Player ID",
b.PLAYER_NAME as "Name of Player",
substr(a.REMARKS,7,8) as "Team of Player",
a.PLAYER_ROLE as "Player Role"
from ipl_team_players a left join ipl_player b
on a.PLAYER_ID = b.PLAYER_ID
where a.PLAYER_ROLE = "Bowler" and a.remarks = "TEAM - MI";

-- 5.	How many all-rounders are there in each team, Display the teams with more than 4 
-- all-rounder in descending order.
-- Table used: 
select * from ipl_team_players;
-- Solution: 
select Team_id,sum(PLAYER_ROLE = "All-Rounder")  as "Number of All-Rounders in a Team"
from ipl_team_players
group by team_id
having sum(PLAYER_ROLE = "All-Rounder") > 4
order by sum(PLAYER_ROLE = "All-Rounder") desc;